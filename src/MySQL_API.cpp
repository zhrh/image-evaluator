...
// param
//	generator_type_num, each generator type have been finded in present queries.
//	query_pr, each query's precision and recall
int CMySQL_API::Result_Table_Evaluate(long long int TaskID,
						  long long int SubmitterID,
						  std::map<int, int> &generator_type_num,
						  std::map<int, std::pair<float, float> > &query_pr)
{
#define RESULT_TABLE_EVALUATE "select Result_FixFFID,Result_SimVal from Result_Table where Result_TaskID=? and Result_SubID=? \
and Result_StTime=0 and Result_EdTime=0"
//#define RESULT_TABLE_EVALUATE "select Result_FixFFID,Result_SimVal from Result_Table where Result_TaskID=? and Result_SubID=? \
and Result_StTime=0 and Result_EdTime=0 order by Result_SimVal desc"

	MYSQL_STMT *stmt;
	MYSQL_BIND bind_param[2];
	long long int taskID,subID;
	//FILE *ftest = fopen("../result/test","a");

	if(!(stmt = mysql_stmt_init(&mysql)))
	{
		#ifdef TEST
		printf("RESULT_TABLE_EVALUATE mysql_stmt_init failed...%s\n",mysql_error(&mysql));
		#endif
		return -1;
	}

	if(mysql_stmt_prepare(stmt, RESULT_TABLE_EVALUATE, strlen(RESULT_TABLE_EVALUATE))!=0)
	{
		#ifdef TEST
		printf("RESULT_TABLE_EVALUATE Param bind Failed...:%s \n",mysql_stmt_error(stmt));
		#endif
	...
	{
		#ifdef TEST
		printf("RESULT_TABLE_EVALUATE Error param bind..: %s",mysql_stmt_error(stmt));
		#endif
		mysql_stmt_close(stmt);
		return -1;
	}
	...
	{
		#ifdef TEST
		printf("RESULT_TABLE_EVALUATE failed:%s\n", mysql_stmt_error(stmt));
		#endif
		mysql_stmt_close(stmt);
		return -1;
	}
	...
	if (affected_rows==0)
	{
		...
		//fprintf(ftest,"No Result Match TaskID=%d,SubmitterID=%d in Result_Table\n",TaskID,SubmitterID);
		//fclose(ftest);
		#endif
		mysql_stmt_close(stmt);
		if(!query_pr.count(TaskID))
		{
			query_pr[TaskID] = std::make_pair(0.0, 0.0);
			return 0;
		}
		else return -1;
	}

	int generator_type = 0, origin_id = 0, finded_num = 0;
	while(!mysql_stmt_fetch(stmt))
	{
		printf("FFID = %d, SimVal = %d\n",ffid, simval);
		//fprintf(ftest,"FFID = %d, SimVal = %d\n",ffid, simval);
		generator_type = ffid / 100000000;
		origin_id = ffid % 100000000;
		if(origin_id == TaskID)
		{
			generator_type_num[generator_type] = generator_type_num[generator_type] + 1;
			++finded_num;
		}
	}
	float precision = static_cast<float>(finded_num) / affected_rows;
	float recall = static_cast<float>(finded_num) / g_kGeneratorType;
	if(!query_pr.count(TaskID))
		query_pr[TaskID] = std::make_pair(precision, recall);
	else 
	{
		mysql_stmt_close(stmt);
		return -1;
...
{
#define IMAGE_TEMP_UPADATE_ID "update image_temp set old_id =? where image_name =?"
	...
	{
		#ifdef TEST
		printf("IMAGE_TEMP_UPADATE_ID stmt initial failed...%s\n",mysql_error(&mysql));
		#endif
		return -1;
	}
	if(mysql_stmt_prepare(stmt,IMAGE_TEMP_UPADATE_ID,strlen(IMAGE_TEMP_UPADATE_ID))!=0)
	{
		#ifdef TEST
		printf("IMAGE_TEMP_UPADATE_ID Param bind Failed...:%s",mysql_stmt_error(stmt));
		#endif
		mysql_stmt_close(stmt);
		return -1;
	}
	...
	{
		#ifdef TEST
		printf(" IMAGE_TEMP_UPADATE_ID failed:%s\n", mysql_stmt_error(stmt));
		#endif
		mysql_rollback(&mysql);
		mysql_real_query(&mysql,MYSQL_SETCOMMIT_ON,strlen(MYSQL_SETCOMMIT_ON));
